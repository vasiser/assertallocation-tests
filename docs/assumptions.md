...
10. **Exact decimal arithmetic** (`Decimal`, not binary floats) — 0.1 has no exact binary representation, and rounding rules must be deterministic (see test case TC-08, where float math computes 0.3 / 0.1 as 2.999… and would produce 2 shares instead of 3).
